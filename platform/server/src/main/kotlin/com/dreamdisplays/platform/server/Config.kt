 ...
 * Different from `Paper` implementation.
 *
 * `NeoForge` server-side configuration. Mirrors [FabricConfig] but resolves the config directory
 * through `FML`'s paths instead of `FabricLoader`.
 */
@NeoForgeOnly
class NeoForgeConfig {
    private val logger = LoggerFactory.getLogger("DreamDisplays/ServerConfig")

    private val configDir: File = FMLPaths.CONFIGDIR.get().resolve("dreamdisplays").toFile()
    private val configFile = File(configDir, "config.toml")
    ...
    private fun createDefaultConfig() {
        if (!configDir.exists()) configDir.mkdirs()
        if (!configFile.exists()) {
            val resource = NeoForgeConfig::class.java.classLoader
                .getResourceAsStream("assets/dreamdisplays/lang/server/config.toml")
                ?: NeoForgeConfig::class.java.classLoader
                    .getResourceAsStream("config.toml")
            if (resource != null) {
                resource.use { Files.copy(it, configFile.toPath()) }
            } else {
    ...
    companion object {
        ...
        private val DEFAULT_CONFIG = """
# Dream Displays configuration
# NeoForge server implementation
# Support: https://discord.gg/uwMMZ2KWk6
