...
/** `NeoForge` implementation of [PlaybackTransport]: v2 envelopes via [NeoForgeV2Networking]. */
@NeoForgeOnly
object NeoForgePlaybackTransport : PlaybackTransport {
    @Volatile
    private var server: MinecraftServer? = null

    /** Binds the running server; called from `ServerStartedEvent`. */
    fun bind(server: MinecraftServer) {
        this.server = server
    }
    ...
    override fun broadcast(display: DisplayData, packet: DreamPacket) {
        val s = server ?: return
        val neoForge = display as? NeoForgeDisplayData ?: return
        val receivers = DisplayManager.getReceivers(neoForge, s).filter { V2PlayerTracker.isV2(it.uuid) }
        if (receivers.isNotEmpty()) NeoForgeV2Networking.send(receivers, packet)
    }
    ...
    override fun sendTo(playerId: UUID, packet: DreamPacket) {
        val s = server ?: return
        val player = s.playerList.getPlayer(playerId) ?: return
        if (V2PlayerTracker.isV2(playerId)) NeoForgeV2Networking.send(listOf(player), packet)
    }

    /** UUIDs of players currently in range of [display] (watch-party nearby / ready-check denominator). */
    override fun nearbyPlayerIds(display: DisplayData): List<UUID> {
        val s = server ?: return emptyList()
        val neoForge = display as? NeoForgeDisplayData ?: return emptyList()
        return DisplayManager.getReceivers(neoForge, s).map { it.uuid }
    }
    ...
    override fun isAdmin(playerId: UUID): Boolean {
        val player = server?.playerList?.getPlayer(playerId) ?: return false
        return NeoForgeServerPacketHandler.isOpLevel2(player)
    }
}
