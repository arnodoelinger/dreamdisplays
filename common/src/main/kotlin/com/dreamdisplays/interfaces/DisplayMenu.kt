...
class DisplayMenu private constructor() : Screen(Component.translatable("dreamdisplays.ui.title")) {
    ...
    //? if >=26 {
    override fun extractRenderState(g: GuiGraphicsExtractor, mouseX: Int, mouseY: Int, delta: Float) = drawMenu(g, mouseX, mouseY, delta)
    private fun bgCompat(g: GuiGraphicsCompat) = extractTransparentBackground(g)
    private fun superDraw(g: GuiGraphicsCompat, mouseX: Int, mouseY: Int, delta: Float) = super.extractRenderState(g, mouseX, mouseY, delta)
    //?} else
    /*override fun render(g: GuiGraphics, mouseX: Int, mouseY: Int, delta: Float) = drawMenu(g, mouseX, mouseY, delta)
    private fun bgCompat(g: GuiGraphicsCompat) = renderTransparentBackground(g)
    private fun superDraw(g: GuiGraphicsCompat, mouseX: Int, mouseY: Int, delta: Float) = super.render(g, mouseX, mouseY, delta)*/

    private fun drawMenu(g: GuiGraphicsCompat, mouseX: Int, mouseY: Int, delta: Float) {
        bgCompat(g)

        val ds = displayScreen
        if (ds == null) {
            superDraw(g, mouseX, mouseY, delta)
            return
        }
    ...
    private fun renderErroredOverlay(g: GuiGraphicsCompat, mouseX: Int, mouseY: Int, delta: Float) {
        ...
        for (line in lines) {
            g.drawText(font, line, this.width / 2 - font.width(line) / 2, y, 0xFFFFFFFF.toInt(), false)
            y += font.lineHeight + 4
        }
    ...
    private fun renderPreviewSection(g: GuiGraphicsCompat, px: Int, py: Int, pw: Int, ph: Int) {
        ...
        popoutButtonWidget?.let {
            ...
            if (popoutDropdownVisible) {
                ...
                g.drawText(font, "Window", ddX + 6, fy, 0xFFFFFFFF.toInt(), false)
                g.drawText(font, "In-game", ddX + 6, fy + ddItemH, 0xFFDDDDDD.toInt(), false)
            }
        }
    ...
    private fun renderTitleOverlay(g: GuiGraphicsCompat, scr: DisplayScreen, x: Int, y: Int, w: Int) {
        ...
        if (isNew) {
            ...
            g.drawText(font, tag, titleX + 3, titleY, 0xFFFFFFFF.toInt(), false)
            titleX += tw + 4
            shown = trimToWidth(title, textW - tw - 4)
        }
        g.drawText(font, shown, titleX, titleY, 0xFFFFFFFF.toInt(), false)
        ...
        g.drawText(font, metaShown, x + padX, boxY + padY + font.lineHeight + padY, 0xFFAAAAAA.toInt(), false)
    }
    ...
    private fun renderSettingsSection(g: GuiGraphicsCompat, px: Int, py: Int, pw: Int, ph: Int) {
        val innerX = px + PANEL_PADDING_X
        val innerY = py + headerHeight()
        val innerW = pw - PANEL_PADDING_X * 2
    ...
    private fun renderRow(
        g: GuiGraphicsCompat, x: Int, y: Int, w: Int, key: String,
        control: AbstractWidget?, reset: ButtonWidget?
    ): Int {
        g.fill(x, y, x + w, y + ROW_H, ROW_BG)
        val label = Component.translatable(key)
        g.drawText(font, label, x + 6, y + ROW_H / 2 - font.lineHeight / 2, 0xFFFFFFFF.toInt(), false)
    ...
    private fun renderTooltips(g: GuiGraphicsCompat, mouseX: Int, mouseY: Int) {
        val scr = displayScreen ?: return
    ...
    private fun drawPanel(g: GuiGraphicsCompat, x: Int, y: Int, w: Int, h: Int, title: String) {
        ...
        g.drawText(font, title, x + PANEL_PADDING_X, y + PANEL_PADDING_Y, 0xFFFFFFFF.toInt(), false)
    }
    ...
    private fun renderModLabel(g: GuiGraphicsCompat, x: Int, y: Int) {
